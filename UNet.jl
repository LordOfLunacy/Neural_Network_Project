...
mutable struct UNetwork
    ...
    function UNetwork(featureChannels::Integer = 4)
        ...
        conv3_1 = Convolution(3, 3, f*2, f*4)
        conv3_2 = Convolution(3, 3, f*4, f*4)
        conv3_3 = Convolution(3, 3, f*4, f*8)
        conv4_1 = Convolution(3, 3, f*4, f*2)
        conv4_2 = Convolution(3, 3, f*2, f*2)
        conv4_3 = Convolution(3, 3, f*2, f*4)
        conv5_1 = Convolution(3, 3, f*2, f)
        conv5_2 = Convolution(3, 3, f, f)
        conv5_3 = Convolution(1, 1, f, 3)

        parameterCount = 3 * 3 * 3 * f + f + 3 * 3 * f * f + 3 * 3 * f * f * 2 + f * 2 + 3 * 3 * f * 2 * f * 2 + f * 2 + 3 * 3 * f * 2 * f * 4 + f * 4
        parameterCount += 3*3*f*4*f*4+f*4 + 3*3*f*4*f*8+f*8 + 3*3*f*4*f*2+f*2 + 3*3*f*2*f*2 + f*2 + 3*3*f*2*f*4 + f*4 + 3*3*f*2*f + f + 3*3*f*f + f + f*3 + f
        println("Weight Count: $parameterCount")

        new(conv1_1, conv1_2, conv2_1, conv2_2, conv3_1, conv3_2, conv3_3, conv4_1, conv4_2, conv4_3, conv5_1, conv5_2, conv5_3)
    end
end
...
mutable struct GradData
    ...
    function GradData(featureChannels::Integer = 4)
        ...
        g3_1 = Convolution(3, 3, f*2, f*4)
        g3_2 = Convolution(3, 3, f*4, f*4)
        g3_3 = Convolution(3, 3, f*4, f*8)
        g4_1 = Convolution(3, 3, f*4, f*2)
        g4_2 = Convolution(3, 3, f*2, f*2)
        ...
        new(g1_1, g1_2, g2_1, g2_2, g3_1, g3_2, g3_3, g4_1, g4_2, g4_3, g5_1, g5_2, g5_3, a1_1, a1_2, a2_1, a2_2, a3_1, a3_2, a3_3, a4_1, a4_2, a4_3, a5_1, a5_2, a5_3)
    end
end
...
mutable struct UNetTrainData
    ...
    o3_1::Array{Float32, 4}
    o3_2::Array{Float32, 4}
    o3_3::Array{Float32, 4}
    #array for depth to space representation
    o3_3d2s::Array{Float32, 4}
    o4_1::Array{Float32, 4}
    o4_2::Array{Float32, 4}
    ...
    d3_1::Array{Float32, 4}
    d3_2::Array{Float32, 4}
    d3_3::Array{Float32, 4}
    d3_3d2s::Array{Float32, 4}
    d4_1::Array{Float32, 4}
    d4_2::Array{Float32, 4}
    ...
    e3_1::Array{Float32, 4}
    e3_2::Array{Float32, 4}
    e3_3::Array{Float32, 4}
    e3Temp::Array{Float32, 4}
    e4_1::Array{Float32, 4}
    e4_2::Array{Float32, 4}
    ...
    function UNetTrainData(net::UNetwork, n::Integer, h::Integer, w::Integer)
        ...
        o3_1 = zeros(Float32, (n, f*4, h÷4, w÷4))
        o3_2 = zeros(Float32, (n, f*4, h÷4, w÷4))
        o3_3 = zeros(Float32, (n, f*8, h÷4, w÷4))
        o3_3d2s = zeros(Float32, (n, f*2, h÷2, w÷2))
        o4_1 = zeros(Float32, (n, f*2, h÷2, w÷2))
        o4_2 = zeros(Float32, (n, f*2, h÷2, w÷2))
        ...
        d3_1 = zeros(Float32, (n, f*4, h÷4, w÷4))
        d3_2 = zeros(Float32, (n, f*4, h÷4, w÷4))
        d3_3 = zeros(Float32, (n, f*8, h÷4, w÷4))
        d3_3d2s = zeros(Float32, (n, f*2, h÷2, w÷2))
        d4_1 = zeros(Float32, (n, f*2, h÷2, w÷2))
        d4_2 = zeros(Float32, (n, f*2, h÷2, w÷2))
        ...
        e3_1 = zeros(Float32, (n, f*4, h÷4, w÷4))
        e3_2 = zeros(Float32, (n, f*4, h÷4, w÷4))
        e3_3 = zeros(Float32, (n, f*8, h÷4, w÷4))
        e3Temp = zeros(Float32, (n, f*4, h÷2, w÷2))
        e4_1 = zeros(Float32, (n, f*2, h÷2, w÷2))
        e4_2 = zeros(Float32, (n, f*2, h÷2, w÷2))
        ...
        s = GradData(f)
        new(o1_1, o1_2a, o1_2b, o2_1, o2_2a, o2_2b, o3_1, o3_2, o3_3, o3_3d2s, o4_1, o4_2, o4_3, o4_3d2s, o5_1, o5_2, o5_3, d1_1, d1_2a, d1_2b, d2_1, d2_2a, d2_2b, d3_1, d3_2, d3_3, d3_3d2s, d4_1, d4_2, d4_3, d4_3d2s, d5_1, d5_2, e1_1, e1_2, e2_1, e2_2, e3_1, e3_2, e3_3, e3Temp, e4_1, e4_2, e4_3, e4Temp, e5_1, e5_2, e5_3, s)
    end
...
function UNetInference!(batch, net::UNetwork, info::UNetTrainData)
    ...
    info.d2_2b = dMaxPool!(info.d2_2b, info.o2_2a)
    info.d2_2b .*= info.d2_2a
    info.o2_2a = ReLU!(info.o2_2a)
    info.o2_2b = MaxPool!(info.o2_2b, info.o2_2a)

    info.o3_1 = Convolve!(info.o2_2b, info.o3_1, net.conv3_1)
    info.d3_1 = dReLU!(info.d3_1, info.o3_1)
    info.o3_1 = ReLU!(info.o3_1)
    
    info.o3_2 = Convolve!(info.o3_1, info.o3_2, net.conv3_2)
    info.d3_2 = dReLU!(info.d3_2, info.o3_2)
    info.o3_2 = ReLU!(info.o3_2)

    info.o3_3 = Convolve!(info.o3_2, info.o3_3, net.conv3_3)
    info.d3_3 = dReLU!(info.d3_3, info.o3_3)
    info.o3_3 = ReLU!(info.o3_3)

    info.o3_3d2s = DepthToSpace!(info.o3_3, info.o3_3d2s)
    info.d3_3d2s = DepthToSpace!(info.d3_3, info.d3_3d2s)
    
    info.o4_1 = Convolve!(cat(info.o2_2a, info.o3_3d2s, dims=2), info.o4_1, net.conv4_1)
    info.d4_1 = dReLU!(info.d4_1, info.o4_1)
    info.o4_1 = ReLU!(info.o4_1)

    info.o4_2 = Convolve!(info.o4_1, info.o4_2, net.conv4_2)
    info.d4_2 = dReLU!(info.d4_2, info.o4_2)
    info.o4_2 = ReLU!(info.o4_2)

    info.o4_3 = Convolve!(info.o4_2, info.o4_3, net.conv4_3)
...
function UNetBackprop!(batch, net::UNetwork, info::UNetTrainData)
    ...
    info.e4_2 = BackConvolve!(info.e4_3, info.e4_2, info.d4_2, net.conv4_3)
    info.e4_1 = BackConvolve!(info.e4_2, info.e4_1, info.d4_1, net.conv4_2)

    #contains both e2_2 data and e3_3 data
    info.e3Temp = BackConvolve!(info.e4_1, info.e3Temp, cat(info.d2_2a, info.d3_3d2s, dims=2), net.conv4_1)

    info.e3_3 = SpaceToDepth!(info.e3Temp[:, size(info.e3Temp, 2)÷2+1:size(info.e3Temp, 2), :, :], info.e3_3)
    info.e3_2 = BackConvolve!(info.e3_3, info.e3_2, info.d3_2, net.conv3_3)
    info.e3_1 = BackConvolve!(info.e3_2, info.e3_1, info.d3_1, net.conv3_2)

    info.e2_2 = MaxPoolingBackConvolve!(info.e3_1, info.e2_2, info.d2_2b, net.conv3_1)
    #add in error from skip connection
    info.e2_2 += info.e3Temp[:, 1:size(info.e3Temp, 2)÷2, :, :]
...
function UNetUpdate!(batch, net::UNetwork, info::UNetTrainData, stepsize = 0.001)
    ...
    info.s.g3_1 = WeightStep!(info.o2_2b, info.e3_1, info.s.g3_1)
    info.s.g3_2 = WeightStep!(info.o3_1, info.e3_2, info.s.g3_2)
    info.s.g3_3 = WeightStep!(info.o3_2, info.e3_3, info.s.g3_3)
    info.s.g4_1 = WeightStep!(cat(info.o2_2a, info.o3_3d2s, dims=2), info.e4_1, info.s.g4_1)
    info.s.g4_2 = WeightStep!(info.o4_1, info.e4_2, info.s.g4_2)
    info.s.g4_3 = WeightStep!(info.o4_2, info.e4_3, info.s.g4_3)
    ...
    info.s.a3_1 = AdamUpdate!(info.s.g3_1, info.s.a3_1)
    info.s.a3_2 = AdamUpdate!(info.s.g3_2, info.s.a3_2)
    info.s.a3_3 = AdamUpdate!(info.s.g3_3, info.s.a3_3)
    info.s.a4_1 = AdamUpdate!(info.s.g4_1, info.s.a4_1)
    info.s.a4_2 = AdamUpdate!(info.s.g4_2, info.s.a4_2)
    ...
    net.conv3_1 = WeightUpdate!(net.conv3_1, info.s.a3_1, stepsize)
    net.conv3_2 = WeightUpdate!(net.conv3_2, info.s.a3_2, stepsize)
    net.conv3_3 = WeightUpdate!(net.conv3_3, info.s.a3_3, stepsize)
    net.conv4_1 = WeightUpdate!(net.conv4_1, info.s.a4_1, stepsize)
    net.conv4_2 = WeightUpdate!(net.conv4_2, info.s.a4_2, stepsize)
...
function test()
    net = UNetwork(6)
    info = UNetTrainData(net, 16, 32, 32)
    noisy = []
    truth = []
    stepsize = 0.0001
    for j = 1:2
        ...
        #end
        for i = 1:16:50000
            info = UNetInference!(noisy[i:i+15, :, :, :], net, info)
            info = UNetBackprop!(truth[i:i+15, :, :, :], net, info)
            net, info = UNetUpdate!(noisy[i:i+15, :, :, :], net, info, stepsize)
            println(sum(abs.(info.o5_3 - truth[i:i+15,:,:,:]))/(16*32*32*3))
        end
    end
